...
#define TOTALBIT 20800 //totale duur 1 bit
#define STARTBITVALUE 16640 //80%
#define BITIS1 15600 //75%
#define BITIS0 10400 //50%
#define STOPBITVALUE 5200 //25%
...
ISR (PCINT2_vect) { // wordt aangeroepen bij logische 1 naar 0 of 0 naar 1 van ontvanger
	...
	if (DDRD & (1<<DDD2)) { // opgaande flank (0 -> 1)
		...
		if (diffcounter >= (STARTBITVALUE - OFFSET) && diffcounter <= (STARTBITVALUE + OFFSET)) { // startbit
			...
			//nog iets doen met start- en stopbit?
...
void IR_send(uint8_t waarde) {
	...
	for (int i=0; i<=7; i++) {
		if (waarde & (1<<i)) { //is bit i 1?
			OCR1B = BITIS1; //wacht tot nieuwe bit
			while(TCNT1 != 0);
		} else { //bit i is 0
			OCR1B = BITIS0;
			while(TCNT1 != 0);
